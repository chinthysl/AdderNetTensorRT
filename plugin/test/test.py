...
def get_adder2d_plugin():
    ...
    for plugin_creator in PLUGIN_CREATORS:
        ...
        if plugin_creator.name == plugin_name:
            weight_field = trt.PluginField("weights", np.ones((128,), dtype=np.float32), trt.PluginFieldType.FLOAT32)
            nbWeights_field = trt.PluginField("nbWeights", np.array([128], dtype=np.int32), trt.PluginFieldType.INT32)
            filterSize_field = trt.PluginField("filterSize", np.array([3], dtype=np.int32), trt.PluginFieldType.INT32)
            nbFilters_field = trt.PluginField("nbfilter", np.array([64], dtype=np.int32), trt.PluginFieldType.INT32)
