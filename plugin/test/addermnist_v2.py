...
class Net(nn.Module):
    ...
    def forward(self, x):
        x = F.max_pool2d(self.adder1(x), kernel_size=2, stride=2)
        x = F.max_pool2d(self.adder2(x), kernel_size=2, stride=2)
        # x = self.bn1(x)
        x = x.view(-1, 800)
        x = F.tanh(self.fc1(x))
        x = self.fc2(x)
...
def main():
    ...
    mnist_model.learn(5)
    torch.save(mnist_model.get_weights(), 'adder_mnist_v2.pth')
    ...
    # # ********Get and save single test image********
